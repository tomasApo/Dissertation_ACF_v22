\raggedright
\section{How do we implement Autocorrelation?}
...
The `for loop' finds the minimum value for each cycle in `cycles\_plot' then adds it to `listforMin'. Once the loop finishes, it has 36 values, one minimum value for each cycle. If we want to select the cycle with the lowest value, variable `minIndex', holds the index value of this cycle. In our case it is index nº 30.

\subsection{Average}
Using the mathematical average approach to find the average peak of all the magnitudes of each cycle. Next step, find the mean the 36 values. For understanding purposes, assume it gives us a value of 340.2, then use Python built in method index() to find the cycle number. Unfortunately the mean or average value calculated might not exist in the actual data. Giving the script an error. Instead we have to alter this approach and find the closest real value to the theoretical mean. This is an approach: 
...
This can be justified due to steering cylinder has a lot of feedback as the machine steer's over rocks and uneven terrain unlike lift or tilt cylinder.
